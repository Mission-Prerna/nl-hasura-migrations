...
--     RETURNS TRIGGER AS $$
-- BEGIN
--     INSERT INTO student_update_logs(student_id, old_record, new_record) values (NEW.id, jsonb_diff_val(row_to_json(old)::jsonb, row_to_json(new)::jsonb), jsonb_diff_val(row_to_json(new)::jsonb, row_to_json(old)::jsonb));
--     RETURN NEW;
-- END;
-- $$ LANGUAGE plpgsql;
--
